...
import { AudioLines, Pause, Play, Github, FolderUp, X, Workflow } from 'lucide-react';
...
const CodeChat = () => {
  ...
  const sendSyntaxAnalysis = async (message) => {
    ...
    // Create the payload with the required structure: code and query.
    const payload = {
      code: codeContent,
      query: message  // Use the message string, not the entire messages array
    };
    ...
    try {
      ...
      // Return the final answer (adjust if you need to use a different property)
