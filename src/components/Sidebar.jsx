...
const Sidebar = ({ isOpen, onClose }) => {
  ...
  const sendWebData = async (e) => {
    ...
    try {
      ...
      // 1. Process website through the endpoint
      ...
      // 2. Also save the URL to Firebase
      ...
      const firebaseRes = await fetch(
        'https://secondmemoryai-default-rtdb.firebaseio.com/websiteData.json', 
        firebaseOptions
      );

      if (!firebaseRes.ok) {
        console.error("Error occurred while saving website details to DB", firebaseRes.statusText);
      } else {
        console.log("Website Data dispatched to DB");
      }

      // Handle the result from the processing endpoint
      if (processingRes.ok) {
        // Successful processing
        toast.success(`Added ${processingData.processed_urls?.length || 1} website(s) to Knowledge Base`, {
          id: loadingToastId,
          duration: 3000,
          style: {
            background: '#4BB543',
